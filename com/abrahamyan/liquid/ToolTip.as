package com.abrahamyan.liquid
{
	...
	public class ToolTip extends MovieClip
	{
		...
		public var showDelay:Number = 1;
		public var showTime:Number = .5;
		public var hideTime:Number = .5;
		public var _showArrow:Boolean = true;
		//
		...
		public function show(pLabel:String="", pXpos:Number=0,pYpos:Number=0):void
		{
			...
			TweenLite.to(this, showTime, { autoAlpha:1, ease:Linear.easeNone, delay: showDelay } );
		...
		private function updatePosition():void
		{
			...
			_verticalDirection = DIRECTION_BOTTOM;
			reSize();
			if(_showArrow){
				...
				this.x = Math.round((_horDirection == DIRECTION_LEFT)?(xpos-bg_mc.rect_mc.width):(xpos));
				this.y = Math.round((_verticalDirection == DIRECTION_TOP)?(ypos - bg_mc.rect_mc.height):(ypos));
			}
		...
		public function hide():void
		{
			...
			TweenLite.killTweensOf(this);
			TweenLite.to(this, hideTime, {autoAlpha:0, ease:Linear.easeNone} );
		}
		...
		/**
		 * if false, arrow wouldn't show
		 */
		public function set showArrow(pArrow:Boolean):void
		{
			_showArrow = pArrow;
			bg_mc.arrow_mc.visible = false;
		}
		/**
		 * returns boolean value if arrow wouldn't be show or not
		 */
